 ...
 */
function uniq(array: Array<string | any>) {
...
export default class ProxySandbox implements SandBox {
  ...
  constructor(name: string, globalContext = window, opts?: { speedy: boolean }) {
    ...
    const proxy = new Proxy(fakeWindow, {
      ...
      has(target: FakeWindow, p: string | number | any): boolean {
      ...
      getOwnPropertyDescriptor(target: FakeWindow, p: string | number | any): PropertyDescriptor | undefined {
        /*
      ...
      ownKeys(target: FakeWindow): ArrayLike<string | any> {
      ...
      deleteProperty: (target: FakeWindow, p: string | number | any): boolean => {
        this.registerRunningApp(name, proxy);
        if (target.hasOwnProperty(p)) {
          // @ts-ignore
          delete target[p];
  ...
  //操作proxy时，会执行registerRunningApp
  // 以同步的方式设置当前活跃的子应用，以异步的方式清楚当前活跃的子应用
